// backend/registerSocketHandlers.ts
import type { Server, Socket } from 'socket.io'

/* ================== Tipos ================== */
type AnchorSide = 'L' | 'R' | 'T' | 'B'
type Anchor = { side: AnchorSide; t: number } // t ∈ [0..1] a lo largo del lado
    ...
    switch (type) {
      ...
      case 'link.update': {
        ...
        if (p.sourceId !== undefined) {
          ...
          if (!('anchorSrc' in p)) L.anchorSrc = null
        }
        if (p.targetId !== undefined) {
          L.targetId = p.targetId
          if (!('anchorTgt' in p)) L.anchorTgt = null
        }
